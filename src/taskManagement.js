...
export const taskManagement = (task)=>{
    ...
    const getTasks = () => {
        const cleanTasks = tasks.filter((element)=> (projectManager.projectExists(element.taskProject)));
        return cleanTasks;
    };

    const getTasksByCategory = (category)=>{
        console.log(`Getting all tasks with category ${category}`);
        const filteredTasks = tasks.filter((element)=>element.taskProject==category);
